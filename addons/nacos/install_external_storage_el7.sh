...
verify_supported() {
  ...
  if [[ -z "${NACOS_MYSQL_HOST}" ]]; then
    error "NACOS_MYSQL_HOST MUST set in environment variable."
  fi

  if [[ -z "${NACOS_MYSQL_PORT}" ]]; then
    error "NACOS_MYSQL_PORT MUST set in environment variable."
  fi

  if [[ -z "${NACOS_MYSQL_USER}" ]]; then
    error "NACOS_MYSQL_USER MUST set in environment variable."
  fi

  if [[ -z "${NACOS_MYSQL_PWD}" ]]; then
    error "NACOS_MYSQL_PWD MUST set in environment variable."
  fi
